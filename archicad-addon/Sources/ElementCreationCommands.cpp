...
GS::ObjectState	CreateElementsCommandBase::Execute (const GS::ObjectState& parameters, GS::ProcessControl& /*processControl*/) const
{
    ...
    const GSErrCode transactionError = ACAPI_CallUndoableCommand ("Create " + elemTypeName, [&] () -> GSErrCode {
        API_Element element = {};
        ...
        if (anyFavoriteName) {
            err = ACAPI_Element_GetDefaults (&defaultsSnapshot, &defaultsSnapshotMemo);
            if (err != NoError) return err;
        }
        bool favoriteApplied = false;
        bool savedAutoTextFlag = false;
        err = ACAPI_AutoText_GetAutoTextFlag (&savedAutoTextFlag);
        if (err != NoError) return err;
        bool setAutoTextFlag = false;
        err = ACAPI_AutoText_ChangeAutoTextFlag (&setAutoTextFlag);
        if (err != NoError) return err;
        bool autoTextChanged = true;
        // Also restore on an early return. Normal completion checks both errors.
        const auto restoreDefaults = [&] () -> GSErrCode {
            if (!favoriteApplied) return NoError;
            API_Element restoreMask;
            ACAPI_ELEMENT_MASK_SETFULL (restoreMask);
            const GSErrCode restoreError = ACAPI_Element_ChangeDefaults (&defaultsSnapshot, &defaultsSnapshotMemo, &restoreMask);
            if (restoreError == NoError) favoriteApplied = false;
            return restoreError;
        };
        const GS::OnExit stateGuard ([&] () {
            restoreDefaults ();
            if (autoTextChanged) ACAPI_AutoText_ChangeAutoTextFlag (&savedAutoTextFlag);
        });
        ...
        for (const GS::ObjectState& data : dataArray) {
            if (anyFavoriteName) {
                ...
                if (hasFavoriteName) {
                    // An API failure may follow partial changes to global defaults.
                    ...
                    if (favoriteErr != NoError) {
                        const GSErrCode restoreError = restoreDefaults ();
                        if (restoreError != NoError) return restoreError;
            ...
            // The tool's current default layer can be hidden (e.g. left that way from the
            // last person who used Archicad interactively) - an element created there would
            // be immediately unselectable and unmodifiable via the API (APIERR_HIDDENLAY on
            // any Change call), with no indication anything is wrong. Fall back to the
            // mandatory "Archicad" layer (index 1, present in every project, essentially
            // always visible) when that happens. SetTypeSpecificParameters below still runs
            // after this and can set its own explicit layerIndex where a command supports
            // one, so explicit caller intent is never silently overridden.
            {
                API_Attribute defaultLayerAttr = {};
                defaultLayerAttr.header.typeID = API_LayerID;
                defaultLayerAttr.header.index = element.header.layer;
                if (ACAPI_Attribute_Get (&defaultLayerAttr) == NoError && (defaultLayerAttr.header.flags & APILay_Hidden) != 0) {
                    API_Attribute archicadLayerAttr = {};
                    archicadLayerAttr.header.typeID = API_LayerID;
                    archicadLayerAttr.header.index = ACAPI_CreateAttributeIndex (1);
                    if (ACAPI_Attribute_Get (&archicadLayerAttr) == NoError && (archicadLayerAttr.header.flags & APILay_Hidden) == 0) {
                        element.header.layer = archicadLayerAttr.header.index;
                    }
                }
            }
        ...
        const GSErrCode defaultsError = restoreDefaults ();
        const GSErrCode autoTextError = ACAPI_AutoText_ChangeAutoTextFlag (&savedAutoTextFlag);
        if (autoTextError == NoError) autoTextChanged = false;
        return defaultsError != NoError ? defaultsError : autoTextError;
    });
    ...
    if (transactionError != NoError) return CreateErrorResponse (transactionError, "Native creation transaction failed; committed elements are not confirmed.");
    return response;
}
            ...
            "columnsData": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "circleBased": {
                            "type": "boolean",
                            "description": "True for a round column cross section, false for rectangular. Rejected for profile sections. Applied to all segments."
                        },
                        ...
                        "slantAngle":{"type":"number","description":"Radians from vertical; omission of isSlanted enables slant for a nonzero angle."},
                        "slantDirectionAngle":{"type":"number","description":"Slant direction in project XY radians."},
                        "isFlipped":{"type":"boolean"},"wrapping":{"type":"boolean"},
                        "relativeTopStory":{"type":"integer","minimum":-32768,"maximum":32767,"description":"Relative top-story link; cannot be combined with explicit height."},
                        "topOffset":{"type":"number","description":"Metres relative to the linked top story."},
                        "cutFillPen":{"$ref":"#/OverriddenPen"},"cutFillBackgroundPen":{"$ref":"#/OverriddenPen"},"coverFill":{"$ref":"#/CoverFill"},
                        "floorIndex": {
                            "type": "integer",
                    ...
                    "not":{"anyOf":[{"required":["height","relativeTopStory"]},{"required":["buildingMaterialId","profileId"]},{"required":["circleBased","profileId"]}]}
                }
            }
        },
        "additionalProperties": false,
...
GS::Optional<GS::ObjectState> CreateColumnsCommand::SetTypeSpecificParameters (API_Element& element, API_ElementMemo& memo, const Stories& stories, const GS::ObjectState& parameters) const
{
    ...
    if (parameters.Contains ("height") && parameters.Contains ("relativeTopStory"))
        return CreateErrorResponse (APIERR_BADPARS, "Choose explicit column height or a top-story link, not both.");
    const GSErrCode sectionError = ConfigureColumnForCreation (element, memo, parameters, stories);
    if (sectionError != NoError) return CreateErrorResponse (sectionError, "Cannot configure column section from the requested fields and target attributes.");
...
GS::Optional<GS::ObjectState> CreateSlabsCommand::SetTypeSpecificParameters (API_Element& element, API_ElementMemo& memo, const Stories& stories, const GS::ObjectState& parameters) const
{
    ...
    const auto settingsError = ConfigureSlabForCreation (element, parameters, stories);
    if (settingsError.HasValue ()) return CreateErrorResponse (APIERR_BADPARS, settingsError.Get ());
                                                  ...
                                                  bool isLamp)
{
            ...
            "%s": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "libraryPart": {"type":"object","properties":{"index":{"type":"integer","minimum":1},"guid":{"type":"string","format":"uuid"},"ownUnID":{"type":"string","minLength":1}},"required":["index","guid"],"additionalProperties":false,"description":"Loaded target-project part. Index and main GUID must match; ownUnID additionally guards the revision."},
                        "coordinates": {
                            "$ref": "#/Coordinate3D"
                        },
                        "dimensions": {
    ...
    schema += GS::UniString::Printf (R"(,
                    ...
                    "oneOf": [{"required":["libraryPartName"]},{"required":["libraryPart"]}]
                }
            }
        },
        "additionalProperties": false,
...
{
    if (params==nullptr || *params==nullptr) return false;
    USize changed=0;
	...
	for (GSIndex ii = 0; ii < nParams; ++ii) {
        ...
        if (actParam.typeMod==API_ParArray) return false;
    ...
    return changed==changeParamsDictionary.GetSize ();
}
...
static GS::Optional<GS::ObjectState> ResolveLibraryPartName (API_Element& element, API_ElementMemo& memo, const GS::ObjectState& parameters)
{
    API_LibPart libPart = {};
    const GS::OnExit dispose ([&] { delete libPart.location; });
    const auto* selected=parameters.Get ("libraryPart");
    GS::UniString name;
    const bool byName=parameters.Get ("libraryPartName",name);
    if ((selected!=nullptr)==byName) return CreateErrorResponse (APIERR_BADPARS,"Select exactly one libraryPart or libraryPartName.");
    const API_LibTypeID expected=GetElemTypeId (element.header)==API_LampID?APILib_LampID:APILib_ObjectID;
    GSErrCode err=NoError;
    if (selected!=nullptr) {
        if (!selected->Get ("index",libPart.index) || libPart.index<1) return CreateErrorResponse (APIERR_BADPARS,"Invalid loaded library index.");
        err=ACAPI_LibraryPart_Get (&libPart);
        if (err!=NoError) return CreateErrorResponse (err,"Selected target-library part is unavailable.");
        const API_Guid guid=GetGuidFromObjectState (*selected);
        GS::UniString ownUnID;
        if (guid==APINULLGuid || GSGuid2APIGuid (GS::UnID (libPart.ownUnID).GetMainGuid ())!=guid ||
            (selected->Get ("ownUnID",ownUnID) && ownUnID!=GS::UniString (libPart.ownUnID)))
            return CreateErrorResponse (APIERR_BADPARS,"Library identity changed; refresh the loaded inventory.");
    } else {
        if (name.IsEmpty ()) return CreateErrorResponse (APIERR_BADPARS,"Library name must not be empty.");
        Int32 count=0,match=0;
        err=ACAPI_LibraryPart_GetNum (&count);
        if (err!=NoError) return CreateErrorResponse (err,"Cannot enumerate loaded parts.");
        for (Int32 index=1;index<=count;++index) {
            API_LibPart candidate = {}; candidate.index=index;
            const GS::OnExit release ([&] { delete candidate.location; });
            err=ACAPI_LibraryPart_Get (&candidate);
            if (err!=NoError) return CreateErrorResponse (err,"Cannot resolve a unique library name because an inventory entry could not be read.");
            if (candidate.typeID==expected && candidate.isPlaceable && !candidate.isTemplate && !candidate.missingDef && GS::UniString (candidate.docu_UName)==name) {
                if (match!=0) return CreateErrorResponse (APIERR_BADPARS,"Library name is ambiguous. Select an index and GUID from SearchLibraryParts.");
                match=index;
            }
        }
        if (match==0) return CreateErrorResponse (APIERR_BADNAME,"No placeable part of the required type has the exact requested name.");
        libPart.index=match; err=ACAPI_LibraryPart_Get (&libPart);
        if (err!=NoError) return CreateErrorResponse (err,"Cannot read selected library part.");
    }
    if (libPart.missingDef || !libPart.isPlaceable || libPart.isTemplate || libPart.typeID!=expected)
        return CreateErrorResponse (APIERR_BADPARS,"Library part is not placeable with the requested Object or Lamp tool.");
    // A favourite owns its parameter memo. Retain it only for the same part;
    // ordinary selection must load the selected part's parameters, not those
    // inherited from whichever object happened to be the current tool default.
    GS::UniString favourite;
    if (parameters.Get ("favoriteName",favourite) && !favourite.IsEmpty ()) {
        if (element.object.libInd!=libPart.index) return CreateErrorResponse (APIERR_BADPARS,"Favourite and library selection refer to different parts.");
    } else {
        double a=0,b=0; Int32 count=0; API_AddParType** selectedParams=nullptr;
        err=ACAPI_LibraryPart_GetParams (libPart.index,&a,&b,&count,&selectedParams);
        if (err!=NoError) { ACAPI_DisposeAddParHdl (&selectedParams); return CreateErrorResponse (err,"Cannot load the selected part's parameters."); }
        ACAPI_DisposeAddParHdl (&memo.params); memo.params=selectedParams;
        element.object.xRatio=a; element.object.yRatio=b;
    }
    ...
    const Stories& stories, const GS::ObjectState& parameters)
{
    ...
    if (parameters.Get ("coordinates", coordinates)) {
        ...
        if (!std::isfinite (apiCoordinate.x) || !std::isfinite (apiCoordinate.y) || !std::isfinite (apiCoordinate.z))
            return CreateErrorResponse (APIERR_BADPARS,"Object coordinates must be finite metres.");
    ...
    if (parameters.Get ("dimensions", dimensions)) {
        ...
        if (!std::isfinite (dims.x) || !std::isfinite (dims.y) || !std::isfinite (dims.z) || dims.x<=0 || dims.y<=0 || dims.z<=0)
            return CreateErrorResponse (APIERR_BADPARS,"Object dimensions must be positive finite metres.");
        ...
        if (!ChangeParams (memo.params, {{"ZZYZX", os}}))
            return CreateErrorResponse (APIERR_BADPARS,"Selected part has no supported scalar ZZYZX height. Configure its discovered GDL parameters instead.");
        if (mask != nullptr) {
...
{
    auto err = ResolveLibraryPartName (element, memo, parameters);
    if (err.HasValue ()) {
        return err;
    }
...
static GS::ObjectState ExecuteModifyObjectOrLamp (const GS::ObjectState& parameters, const char* arrayFieldName, API_ElemTypeID elemTypeID, const char* undoableCommandName)
{
    ...
    const GSErrCode transactionError = ACAPI_CallUndoableCommand (undoableCommandName, [&] () -> GSErrCode {
        for (const GS::ObjectState& item : itemsWithDetails) {
            ...
            if (err != NoError) {
                executionResults (CreateFailedExecutionResult (err, "Cannot read library parameters before modifying the element."));
                continue;
            }
            ...
            executionResults (err == NoError ? CreateSuccessfulExecutionResult () : CreateFailedExecutionResult (err, DescribeElementChangeFailure (err, "Failed to modify element.")));
        }
...
GS::ObjectState ModifyTextsCommand::Execute (const GS::ObjectState& parameters, GS::ProcessControl& /*processControl*/) const
{
    ...
    const GSErrCode transactionError = ACAPI_CallUndoableCommand ("ModifyTexts", [&] () -> GSErrCode {
        for (const GS::ObjectState& item : itemsWithDetails) {
            const GS::ObjectState* elementId = item.Get ("elementId");
            if (elementId == nullptr) {
                executionResults (CreateFailedExecutionResult (APIERR_BADPARS, "elementId is missing"));
...
GS::ObjectState ModifyLabelsCommand::Execute (const GS::ObjectState& parameters, GS::ProcessControl& /*processControl*/) const
{
    ...
    const GSErrCode transactionError = ACAPI_CallUndoableCommand ("ModifyLabels", [&] () -> GSErrCode {
        for (const GS::ObjectState& item : itemsWithDetails) {
            const GS::ObjectState* elementId = item.Get ("elementId");
            if (elementId == nullptr) {
                executionResults (CreateFailedExecutionResult (APIERR_BADPARS, "elementId is missing"));
